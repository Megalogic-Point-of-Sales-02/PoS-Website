  ...
  useEffect(() => {
    const getSalesForecastingPrediction = async () => {
      ...
      for (let i = 0; i < predictionResult.length; i++) {
        ...
        salesData.push({ x: date.toISOString().slice(0, 10), y: predictionResult[i] });
      }

      setApexChartData(salesData);
      setIsLoading(false);
    };
    getSalesForecastingPrediction();
  }, [time]);
      ...
      yaxis: {
        ...
        labels: {
          ...
          formatter: function (val) {
            return (val / 1000000).toFixed(0); // Ganti menjadi per berapanya
          },
          style: {
            colors: "#ffffff",
            fontSize: "12px",
