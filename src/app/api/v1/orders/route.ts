...
export async function POST(req: NextRequest) {
  ...
  const { data: insertData, error: insertError } = await supabase.from("orders").insert([requestBody]).select();

  // Error
  if (insertError) {
    return new NextResponse(JSON.stringify(insertError.message), {
      status: 500,
    });
  }

  // Update customers total spend
  const { data: incrementData, error: incrementError } = await supabase.rpc("increment_total_spend", { customer_id: requestBody.customer_id, increment_spend: requestBody.sales });

  // Error
  if (incrementError) {
    return new NextResponse(JSON.stringify(incrementError.message), {
      status: 500,
    });
  }

  // Success
  return new NextResponse(JSON.stringify(insertData), {
    status: 200,
  });
}
